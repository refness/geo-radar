...
### Security checklist for forks and contributors

This repository is already public — the checklist below is for **anyone who forks this project** and develops further on their own copy before making *their* fork public (or for reference if you ever reset this repo's history yourself).

If a real API key was ever committed at some point during development (even if it was later removed from the latest commit), **it's still recoverable from the repository's git history** once that history is public on GitHub. Before making a fork public:
...
3. `package.json`'s `build.publish` section has `owner`/`repo` filled in for this repository — update them to your own if you fork this project.
4. Verified: at the time this repository was prepared for publication, no real keys were found in the code or git history — but that check doesn't replace running your own `git log -p | grep -i key`.
